...
export function SpiderExperience() {
  ...
  return <main className={`spidey-universe ${sense ? 'spider-sense-active' : ''}`}>
    ...
    <SuitVault/>
    <section id="mechanics" className="mechanics-section relative z-10 border-y border-border px-5 py-28 text-foreground md:px-10 lg:px-16"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.4fr_.6fr]"><div><p className="font-mono text-xs tracking-[.3em] text-primary">WEB-SHOOTER PROTOCOL</p><h2 className="mt-3 text-5xl font-black italic leading-none md:text-7xl">PHYSICS &<br/>WEB SHOOTER</h2><div className="lab-hud mt-10 overflow-hidden border border-[var(--web-cyan)]"><div className="flex items-center justify-between border-b border-[var(--web-cyan)]/30 px-4 py-2 font-mono text-[10px] tracking-[.2em] text-[var(--web-cyan)]"><span>STARK // ELASTICITY SIMULATOR</span><span>CORE ONLINE</span></div><ElasticWebLab/></div><p className="mt-3 font-mono text-xs opacity-70">DRAG THE HOLOGRAPHIC NODE. RELEASE TO TEST ELASTIC RESPONSE.</p></div><aside className="flex flex-col justify-end gap-4"><div className="stat-block"><span>TENSILE STRENGTH</span><strong>950</strong><em>PSI</em></div><div className="stat-block"><span>SWING VELOCITY</span><strong>120</strong><em>MPH</em></div><div className="border border-current p-5 font-mono text-xs leading-relaxed"><Volume2 className="mb-3" aria-hidden="true"/>CLICK ANYWHERE TO FIRE A PROCEDURAL WEB STRAND.</div></aside></div></section>
